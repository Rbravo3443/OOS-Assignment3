...
public class Planet {
	...
	public Planet(String planetName, String diameterkm, String temperatureF, int Numberofmoon)
			throws InvalidPlanetException {
		try {
	...
	public boolean isValidPlanet(String PlanetName) {
		...
		for (int i = 0; i < PlanetName.length(); i++) {
			...
			if ((isValidNameLength(PlanetName)) && (Character.isDigit(c) || Character.isLetter(c)
	...
	public boolean isValidFields(String PlanetName, String testDiameter, String temperature, int moons) {
		boolean valid = false;
		if (isValidPlanet(PlanetName) && isValiddiameterNumber(testDiameter) && isValidTemperature(temperature)
				&& isValidNumberofMoons(moons)) {
			valid = true;
		} else {
			valid = false;
		}
